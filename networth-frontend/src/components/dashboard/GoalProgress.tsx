...
const GoalProgress: React.FC<GoalProgressProps> = ({ currentNetWorth, currency, secondaryGoalsData }) => {
        ...
        try {
            ...
            if (fetchedGoals && fetchedGoals.length > 0) {
                ...
                if (primary) {
                    ...
                    // Convert stored AED value to Display Currency for editing
                    const displayAmount = convert(primary.targetAmount, 'AED');
                    setEditGoalValue(displayAmount.toString());
    ...
    const handleSaveGoal = async () => {
        ...
        try {
            ...
            // Convert Input (Display Currency) to Storage (AED)
            // If we are in USD, input 1000 USD -> save 3670 AED
            const amountInAED = convertRaw(parseFloat(editGoalValue), currency.code, 'AED');

            const payload = {
                name: 'Primary Net Worth Goal',
                type: 'NETWORTH',
                targetAmount: amountInAED,
                targetDate: new Date(editGoalDate).toISOString(),
                // currentNetWorth passed in is already converted to Display, but API might expect raw?
                // Actually currentAmount in payload is likely for history tracking. 
                // Let's make sure we send AED for consistency if the backend expects base currency.
                // Re-calculating raw net worth from props is hard without inverse.
                // Optimally, we should pass raw net worth too, but for now let's assume this field is just snapshot.
                // We'll proceed with sending the converted value back to AED if we can, 
                // but since we only have 'currentNetWorth' (Display), let's convert it back to AED.
                currentAmount: convertRaw(currentNetWorth, currency.code, 'AED')
            };
