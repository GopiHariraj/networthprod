                    ...
                    <div className="flex flex-col md:flex-row items-center justify-between gap-4">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p className="text-sm text-slate-500">Securely pass on read-only access to your assets if inactive.</p>
                            </div>
                        </div>
                        {user?.planType === 'PRO' || user?.planType === 'ENTERPRISE' ? (
                            <button
                                onClick={() => window.location.href = '/settings/nominee'}
                                className="px-6 py-2.5 bg-indigo-50 hover:bg-indigo-100 dark:bg-indigo-900/30 dark:hover:bg-indigo-900/50 text-indigo-700 dark:text-indigo-300 font-semibold rounded-xl transition-colors whitespace-nowrap"
                            >
                                Configure Options ➔
                            </button>
                        ) : (
                            <button
                                disabled
                                className="px-6 py-2.5 bg-slate-50 text-slate-400 font-semibold rounded-xl transition-colors whitespace-nowrap opacity-50 cursor-not-allowed"
                            >
                                Upgrade to Unlock 🔒
                            </button>
                        )}
                    ...
                    <div className="space-y-4">
                        <div className="flex items-center justify-between p-4 bg-slate-50 dark:bg-slate-900/50 rounded-xl">
                            <div>
                                ...
                                <div className="text-sm text-slate-500">Receive financial updates and alerts</div>
                            </div>
                            <button
                                disabled={user?.planType !== 'PRO' && user?.planType !== 'ENTERPRISE'}
                                onClick={() => setSettings({ ...settings, notifications: !settings.notifications })}
                                className={`relative w-14 h-8 rounded-full transition-colors ${(user?.planType !== 'PRO' && user?.planType !== 'ENTERPRISE') ? 'bg-slate-200 dark:bg-slate-700 cursor-not-allowed opacity-50' : settings.notifications ? 'bg-blue-600 cursor-pointer' : 'bg-slate-300 dark:bg-slate-600 cursor-pointer'
                                    }`}
                            >
