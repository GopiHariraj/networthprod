...
export default function GoalsPage() {
        ...
        try {
            ...
            if (fetchedGoals && fetchedGoals.length > 0) {
                ...
                if (primaryGoal) {
                    setActiveGoal({
                        id: primaryGoal.id,
                        // Convert stored AED to Display Currency
                        goalNetWorth: convert(primaryGoal.targetAmount, 'AED').toString(),
                ...
                setGoals(prev => ({
                    ...prev,
                    // Convert secondary goals to Display Currency (except Gold which is grams)
                    commodityGrams: goldGoal?.targetAmount.toString() || '',
                    propertyValue: propertyGoal ? convert(propertyGoal.targetAmount, 'AED').toString() : '',
                    stocks: stocksGoal ? convert(stocksGoal.targetAmount, 'AED').toString() : '',
                    cashAndBank: cashGoal ? convert(cashGoal.targetAmount, 'AED').toString() : '',
                    bonds: bondsGoal ? convert(bondsGoal.targetAmount, 'AED').toString() : '',
                    monthlyIncome: incomeGoal ? convert(incomeGoal.targetAmount, 'AED').toString() : '',
                    monthlyExpense: expenseGoal ? convert(expenseGoal.targetAmount, 'AED').toString() : '',
                }));
            }
        } catch (e) {
    ...
    const calculateProgress = () => {
        // activeGoal.goalNetWorth is now in Display Currency
        ...
        // Convert currentNetWorth (AED) to Display Currency
        const currentNetWorthDisplay = convert(currentNetWorth, 'AED');

        const progressPercentage = (currentNetWorthDisplay / goal) * 100;
        const remaining = goal - currentNetWorthDisplay;
    ...
    const saveActiveGoal = async () => {
        if (activeGoal.goalNetWorth && activeGoal.targetDate) {
            const payload = {
                ...
                type: 'NETWORTH',
                // Convert Input (Display) -> AED
                targetAmount: convertRaw(parseFloat(activeGoal.goalNetWorth), currency.code, 'AED'),
                targetDate: new Date(activeGoal.targetDate).toISOString(),
                notes: activeGoal.notes,
                currentAmount: networthData.netWorth // Store raw AED networth
            };
            ...
            for (const map of secondaryMappings) {
                if (map.value) {
                    ...
                    const payload = {
                        name: map.name,
                        type: map.type,
                        // Convert Input (Display) -> AED for monetary values. Gold is grams (no conversion req if raw).
                        // Note: Gold input is stored in 'value', if it's grams, convertRaw might mess it up if we pass currency code.
                        // map.type === 'GOLD' ? parseFloat(map.value) : convertRaw(parseFloat(map.value), currency.code, 'AED')
                        targetAmount: map.type === 'GOLD'
                            ? parseFloat(map.value)
                            : convertRaw(parseFloat(map.value), currency.code, 'AED'),
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                            ...
                            {goals.propertyValue && (
                                <div className="text-center p-3 bg-blue-50 dark:bg-blue-900/10 rounded-xl">
                                    ...
                                    <div className="font-bold text-slate-900 dark:text-white">{currency.symbol} {parseFloat(goals.propertyValue).toLocaleString()}</div>
                                </div>
                            )}
                            {goals.monthlyIncome && (
                                <div className="text-center p-3 bg-green-50 dark:bg-green-900/10 rounded-xl">
                                    ...
                                    <div className="font-bold text-slate-900 dark:text-white">{currency.symbol} {parseFloat(goals.monthlyIncome).toLocaleString()}</div>
                                </div>
                            )}
                            {goals.cashAndBank && (
                                <div className="text-center p-3 bg-emerald-50 dark:bg-emerald-900/10 rounded-xl">
                                    ...
                                    <div className="font-bold text-slate-900 dark:text-white">{currency.symbol} {parseFloat(goals.cashAndBank).toLocaleString()}</div>
                                </div>
                            )}
                            {goals.stocks && (
                                <div className="text-center p-3 bg-purple-50 dark:bg-purple-900/10 rounded-xl">
                                    ...
                                    <div className="font-bold text-slate-900 dark:text-white">{currency.symbol} {parseFloat(goals.stocks).toLocaleString()}</div>
                                </div>
                            )}
                            {goals.bonds && (
                                <div className="text-center p-3 bg-indigo-50 dark:bg-indigo-900/10 rounded-xl">
                                    ...
                                    <div className="font-bold text-slate-900 dark:text-white">{currency.symbol} {parseFloat(goals.bonds).toLocaleString()}</div>
                                </div>
                            )}
                            {goals.monthlyExpense && (
                                <div className="text-center p-3 bg-red-50 dark:bg-red-900/10 rounded-xl">
                                    ...
                                    <div className="font-bold text-slate-900 dark:text-white">{currency.symbol} {parseFloat(goals.monthlyExpense).toLocaleString()}</div>
                                </div>
                            )}
                        </div>
                    </div>
                                    ...
                                    <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                                        ...
                                        <div className="grid grid-cols-3 md:grid-cols-5 gap-2 md:gap-4">
                                            <div className="text-center">
                                                ...
                                                <div className="text-sm md:text-lg font-bold text-white">{currency.symbol} {convert(currentNetWorth, 'AED').toLocaleString()}</div>
                                            </div>
                                            <div className="text-center">
